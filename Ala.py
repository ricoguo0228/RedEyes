    ...
    def remove_red_eyes(self, point1, point2):
        ...
        for i in range(min_y, min_y + height + 1):
            for j in range(min_x, min_x + width + 1):
                if (self.H[i, j] < math.pi / 4) or self.H[i, j] > math.pi * 7 / 4:
                    if self.S[i, j] > 0.3:
                        self.S[i, j] = 0
        self.hsi2rgb()
    ...
    def hsi2rgb(self):
        ...
        for i in range(self.H.shape[0]):
            for j in range(self.H.shape[1]):
                if self.H[i, j] <= mid:
                    self.R[i, j] = self.I[i, j] * (
                            1 + self.S[i, j] * math.cos(self.H[i, j]) / math.cos(0.5 * mid - self.H[i, j]))
                    self.B[i, j] = self.I[i, j] * (1 - self.S[i, j])
                    self.G[i, j] = 3 * self.I[i, j] - (self.R[i, j] + self.B[i, j])
                elif (self.H[i, j] <= mid * 2) and self.H[i, j] > mid:
                    self.H[i, j] = self.H[i, j] - mid
                    self.G[i, j] = self.I[i, j] * (
                            1 + self.S[i, j] * math.cos(self.H[i, j]) / math.cos(0.5 * mid - self.H[i, j]))
                    self.R[i, j] = self.I[i, j] * (1 - self.S[i, j])
                    self.B[i, j] = 3 * self.I[i, j] - (self.R[i, j] + self.G[i, j])
                elif (self.H[i, j] <= mid * 3) and self.H[i, j] > mid * 2:
                    self.H[i, j] = self.H[i, j] - mid * 2
                    self.B[i, j] = self.I[i, j] * (
                            1 + self.S[i, j] * math.cos(self.H[i, j]) / math.cos(0.5 * mid - self.H[i, j]))
